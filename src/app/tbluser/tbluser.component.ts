...
@Component({
  selector: 'app-tbluser',
  templateUrl: './tbluser.component.html',
  styleUrls: ['./tbluser.component.css']
})

export class TbluserComponent implements AfterViewInit {
  ...
  displayedColumns: string[] = ['Build_ID', 'Start_DateTimeStamp', 'Portfolio_Name', 'ValueStream_Name', 'CI_Application_Name', 'Test_Environment', 'Build_Status', 'Predicted_RCA', 'Actual_RCA', 'RCA_Review_Status', 'action'];
  ...
  //===============================================================================================
  @ViewChild('child', { static: false }) child:TbluseractivityComponent;
  ...
  onApprove(build_Id) {
    ...
    this._httpClient.post<any>('http://localhost:8080/api/approve', { "id": build_Id }).subscribe(data => { this.refreshData(); this.child.refreshData();})
